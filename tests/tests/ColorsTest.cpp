...
TEST_CASE("Can generate random color.", "[Colors::RGB]") {
	...
	newColor = Colors::generateRandomColor();
...
TEST_CASE("Can generate random color array.", "[Array]") {
    ...
    Colors::generateRandomColorArray(newArray, &Colors::BLUE, numColors, range);
...
TEST_CASE("Can generate scaling color array.", "[Array]") {
	...
	Colors::generateScalingColorArray(newArray, &Colors::BLACK, &Colors::WHITE, numColors, false);
...
TEST_CASE("Can mix colors.", "[Colors::RGB]") {
    ...
    SECTION("Color multiplied by half equals halved values.") {
		...
		Colors::RGB mixed = Colors::mixColors(&color, &halfWhite, Colors::MixMode::MULTIPLY);
	...
	SECTION("Colors blend with specified alpha.") {
		Colors::RGB mixed = Colors::mixColors(&Colors::BLACK, &Colors::WHITE, Colors::MixMode::ALPHA_BLENDING, 0.2);
