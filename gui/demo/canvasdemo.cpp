...
CanvasDemo::CanvasDemo(QWidget* parent, MaestroController* maestro_controller) : SimpleDrawingArea(parent, maestro_controller) {
	...
	cursor->set(40, 0);
	target->set(40, 80);
	canvas->draw_line(cursor, target);
	...
	cursor->set(7, 10);
	canvas->draw_text(cursor, font, "Pixel");

	cursor->set(2, 20);
	canvas->draw_text(cursor, font, "Maestro");
	...
	cursor->set(60, 20);
	canvas->draw_circle(cursor, 5, true);
	canvas->draw_circle(cursor, 10, false);
	canvas->draw_circle(cursor, 15, false);
	canvas->draw_circle(cursor, 19, false);

	// Draw triangles in the bottom-left corner
	cursor->set(11, 71);
	target->set(31, 51);
	canvas->draw_triangle(cursor, target, new Point(26, 66), true);

	cursor->set(9, 69);
	//target->set(9, 49);
	target->set(14, 54);
	canvas->draw_triangle(cursor, target, new Point(29, 49), true);
	...
	cursor->set(7, 47);
	canvas->draw_rect(cursor, new Point(27, 27), false);
