...
## Other Methods
* `Section::getPixelIndex(unsigned short row, unsigned short column)`: Translates a Pixel's x and y coordinates into its index in the Pixel array.
* `Section::setAll(Colors::RGB \*color)`: Sets all Pixels to the specified color.
* `Section::setOne(unsigned ing index, Colors::RGB \*color)`: Sets the Pixel at the specified array index to the specified color.
