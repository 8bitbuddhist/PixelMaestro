...
namespace PixelMaestro {
	...
	*/
	Colors::RGB Section::get_pixel_color(uint16_t x, uint16_t y, Colors::RGB* base_color) {
		...
		/*
		 * If we have Canvas, use the color generated by the Canvas.
		 * Otherwise, use the Pixel's current color.
		 */
		if (canvas_ != nullptr) {
			final_color = canvas_->get_pixel_color(offset_x, offset_y);
		}
		else {
	...
	/**
	 * Mirrors the Sectoin across the axes.
	...
	*/
	void Section::update(const uint32_t& current_time) {
		 ...
		 * Canvases and Scrolls don't need to be done in any specific order.
		 */
		if (layer_ != nullptr) {
