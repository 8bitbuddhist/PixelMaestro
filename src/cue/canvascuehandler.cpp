...
namespace PixelMaestro {
	...
	uint16_t CanvasCueHandler::start_cue(uint8_t handler_byte, uint8_t action_byte, uint8_t section_num, uint8_t layer_num, uint16_t frame_num) {
		...
		IntByteConvert frame_bytes(frame_num);

		controller_.get_buffer()[++index] = frame_bytes.converted_0;	// Byte::FrameByte1
		controller_.get_buffer()[++index] = frame_bytes.converted_1;	// Byte::FrameByte2
		return index;
	}
	...
	void CanvasCueHandler::run(uint8_t *cue) {
		...
		uint16_t frame_index = IntByteConvert::byte_to_int(&cue[(uint8_t)Byte::FrameByte1]);
